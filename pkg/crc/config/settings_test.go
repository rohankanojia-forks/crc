...
func TestSetPullSecretStoredInKeyringIsSetInConfig(t *testing.T) {
	// Given
	cfg, err := newInMemoryConfig()
	require.NoError(t, err)

	// When
	_, err = cfg.Set(PullSecretStoredInKeyring, true)
	require.NoError(t, err)
	...
	assert.Equal(t, SettingValue{
		Value:   true,
		Invalid: false,
	}, cfg.Get(PullSecretStoredInKeyring))
}
...
var configDefaultValuesTestArguments = []struct {
	...
	{
		PullSecretStoredInKeyring, false,
	},
	{
		CPUs, uint(4),
	},
...
var configProvidedValuesTestArguments = []struct {
	...
	{
		PullSecretStoredInKeyring, true,
	},
	{
		CPUs, uint(8),
	},
